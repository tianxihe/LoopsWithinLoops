...
def draw_L(window, circle, r, c):
    ...
    for k in range(r+3):
        ...
        for j in range(3):
            p=rg.Point(circle.center.x+2*j*circle.radius,y)
            cir=rg.Circle(p,circle.radius)
    ...
    for k in range(3):
        ...
        for j in range(c):
            p = rg.Point(cir.center.x + 2 * j * cir.radius+2*cir.radius, y)
            cirr = rg.Circle(p, circle.radius)
